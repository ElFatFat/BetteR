...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(!empty($data->tag) && !empty($data->tagToUnfollow)){
        // On nettoie les données envoyées
        $tag = strip_tags($data->tag);
        $tagToUnfollow = strip_tags($data->tagToUnfollow);
        ...
        $sql = "SELECT `user_id` FROM `user` WHERE `tag` = '".$tagToUnfollow."';";
        $result = $conn->query($sql);
        //On vérifie que l'utilisateur 2 existe.
        if (mysqli_num_rows($result) > 0) {
            //Un uitlisateur avec ce tag existe. On récupère son id.
            $row = mysqli_fetch_assoc($result);
            $idToUnfollow = $row['user_id'];
        }else {
            http_response_code(400);
            exit();
        }
        ...
        $sql = "SELECT * FROM `follow` WHERE `followee` = '".$idToUnfollow."' AND `follower` = '".$id."';";
        $result = $conn->query($sql);
        if (mysqli_num_rows($result) > 0) {
            //L'utilisateur ne suit pas l'utilisateur ciblé.
            $sql = "DELETE FROM `follow` WHERE `followee` = '".$idToUnfollow."' AND `follower` = '".$id."';";
            $result = $conn->query($sql);
            if ($result) {
