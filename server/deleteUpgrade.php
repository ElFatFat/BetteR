...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(!empty($data->tag) && !empty($data->id) && !empty($data->upgrade)){
        ...
        if (mysqli_num_rows($result) > 0) {
            $sql = "DELETE FROM `upgrades` WHERE `user_id` = '".$id."' AND `upgrade_id`= '".$upgrade."';";
            $result = $conn->query($sql);
            http_response_code(200);
            echo json_encode(["message" => "Amélioration supprimée avec succès !"]);
        }else {
            http_response_code(403);
            echo json_encode(["error" => "L'utilisateur ".$tag." ayant l'id ".$id." ne posséde pas l'".$upgrade.""]);
